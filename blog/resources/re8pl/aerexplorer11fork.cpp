// ==WindhawkMod==
// @id              aerexplorer-w11
// @name            Aerexplorer (Windows 11 version)
// @description     Various tweaks for Windows Explorer to make it more like older versions.
// @version         1.6.5
// @author          aubymori
// @github          https://github.com/aubymori
// @include         *
// @compilerOptions -lgdi32 -lcomctl32 -lole32 -loleaut32 -luxtheme -ldwmapi -lntdll -DWINVER=0x0A00
// ==/WindhawkMod==
...
{
    if (WinBuild >= 18363 && settings.smalladdress && uMsg == WM_WINDOWPOSCHANGING && ModernSearchFeatureEnabled())
    {
...
)
{
    ...
    if (WinBuild >= 18363 && settings.smalladdress)
    {
...
std::unordered_map<HWND, TVITEMEXW> oldtvi;
...
void HandleTvItem(HWND hTreeView, LPTVITEMEXW lptvi)
{
    ...
    else
    {
        if (lptvi->iReserved == 1)
        {
            ...
            if (bIsFirst && oldtvi.contains(hTreeView) && oldtvi[hTreeView].hItem != lptvi->hItem) {
                TreeView_SetItem(hTreeView, &oldtvi[hTreeView]);
            }
            if (bIsFirst) {
                memcpy(&oldtvi[hTreeView], lptvi, sizeof(TVITEMEXW));
            }
        }
        else
        {
...
{ 
    ...
    if (hKey == HKEY_LOCAL_MACHINE && 0 == wcscmp(lpSubKey, L"Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Desktop\\NameSpace"))
    {
        CObjectArray_AddItemsFromKeySkip_orig(
            pThis, hKey, L"Software\\Windhawk\\Engine\\ModsWritable\\RegistryRedirect\\HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Desktop\\NameSpace", uUnused, pGuid
        );
    }

    if (hKey == HKEY_LOCAL_MACHINE && 0 == wcscmp(lpSubKey, L"Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\ControlPanel\\NameSpace"))
    {
        CObjectArray_AddItemsFromKeySkip_orig(
            pThis, hKey, L"Software\\Windhawk\\Engine\\ModsWritable\\RegistryRedirect\\HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\ControlPanel\\NameSpace", uUnused, pGuid
        );
    }
...
{
    if (WinBuild >= 18363 && settings.smalladdress && nIndex == SM_CYFIXEDFRAME)
    {
